...
export default function AboutSection() {
  ...
  const getMindsetIcon = (name: string) => {
    switch (name) {
      case 'Code':
        return <Code className="w-4 h-4 text-[#D94E28]" />;
      case 'Smartphone':
        return <Smartphone className="w-4 h-4 text-[#D94E28]" />;
      case 'Layers':
        return <Layers className="w-4 h-4 text-[#D94E28]" />;
      case 'ShieldCheck':
        return <ShieldCheck className="w-4 h-4 text-[#D94E28]" />;
      case 'Target':
        return <Target className="w-4 h-4 text-[#D94E28]" />;
      case 'Sparkles':
      default:
        return <Sparkles className="w-4 h-4 text-[#D94E28]" />;
    }
  };
    ...
    >
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-4 pb-8 border-b border-[#E2DDD2]">
        <div className="flex items-baseline gap-3">
          <span className="text-sm font-mono text-[#D94E28] font-bold">§03</span>
      ...
      <div className="py-8">
        <p className="text-xl sm:text-2xl text-[#141413] font-sans font-medium leading-relaxed max-w-4xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-0 pt-4 pb-12 border-b border-[#E2DDD2]">
            ...
            return (
              <button
                ...
                className={`w-full text-left p-4 rounded-xl transition-all duration-200 flex items-start gap-3 border ${
                  isActive
                    ? 'bg-[#EFEBE1] border-transparent border-l-4 border-l-[#D94E28]'
                    : 'bg-transparent border-transparent hover:bg-[#EFEBE1]/40 text-[#5E5B54]'
                }`}
              >
                <span className={`text-xs font-mono font-bold mt-0.5 ${isActive ? 'text-[#D94E28]' : 'text-[#9C968B]'}`}>
                  {area.number}
                </span>
                <span className={`text-sm sm:text-base font-bold font-display ${isActive ? 'text-[#141413]' : 'text-[#5E5B54]'}`}>
                  {area.title}
                </span>
              </button>
            );
        ...
        <div className="lg:col-span-7 p-2 sm:p-4 lg:p-8 lg:border-l border-[#E2DDD2] flex flex-col justify-between">
          <AnimatePresence mode="wait">
            ...
            >
              <div className="space-y-3">
                <span className="text-xs font-mono font-bold tracking-widest text-[#D94E28] uppercase">
                  {activeArea.tag}
                </span>
                <h3 className="text-2xl font-bold font-display text-[#141413]">
                  {activeArea.title}
                </h3>
              ...
              {/* Tech Chips */}
              <div className="pt-4 border-t border-[#E2DDD2] space-y-2">
                <span className="text-[11px] font-mono uppercase tracking-wider text-[#6E6A61] block">
                ...
                <div className="flex flex-wrap gap-2">
                  {activeArea.skills.map((skill, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1 rounded-lg bg-[#EAE5DA] border border-[#D6CEBF] text-xs font-mono text-[#141413] font-medium"
                    >
                      {skill}
                    </span>
                  ))}
      ...
      <div className="mt-14 space-y-8">
        <div>
          <span className="text-xs font-mono text-[#D94E28] font-bold uppercase tracking-widest block">
            CORE PHILOSOPHY
          </span>
          <h3 className="text-2xl sm:text-3xl font-extrabold text-[#141413] font-display mt-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {MINDSET_PILLARS.map((pillar) => (
            <div
              key={pillar.id}
              className="group relative p-4 sm:p-5 rounded-2xl border border-[#DCD6C8] bg-transparent hover:bg-[#EFEBE1] hover:border-[#D94E28]/40 transition-all duration-300 overflow-hidden"
            >
              ...
              <div className="relative z-10 pl-2 space-y-3">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-lg bg-[#EAE5DA] border border-[#D6CEBF] shrink-0 text-[#141413] group-hover:text-[#D94E28] transition-colors">
                    {getMindsetIcon(pillar.iconName)}
                  </div>
                  <h4 className="text-sm sm:text-base font-bold font-display text-[#141413] group-hover:text-[#D94E28] transition-colors leading-tight">
                    {pillar.title}
                  </h4>
                </div>
                <p className="text-xs sm:text-sm text-[#5E5B54] leading-relaxed pl-1">
                  {pillar.description}
                </p>
              </div>
            </div>
